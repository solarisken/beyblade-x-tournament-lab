...
const BASE_BUILDS=[
{id:"build-ds",name:"DranStrike 1-50 Low Rush",blade:"DranStrike",ratchet:"1-50",bit:"Low Rush",role:"Primary attacker",status:"Tournament Candidate",slot:"Bey 1"},
{id:"build-sw",name:"SilverWolf 9-60 Hexa",blade:"SilverWolf",ratchet:"9-60",bit:"Hexa",role:"Defense",status:"Benchmark",slot:"Bey 2"},
{id:"build-pw",name:"PhoenixWing 9-60 Gear Flat",blade:"PhoenixWing",ratchet:"9-60",bit:"Gear Flat",role:"Secondary attacker",status:"Benchmark",slot:"Bey 3"},
{id:"build-bb",name:"BahamutBlitz BK 0-70 Ignition",blade:"BahamutBlitz BK",ratchet:"0-70",bit:"Ignition",role:"Balance / counter",status:"Test Bey",slot:"Test Bey"}];

const HIST=[
{id:"hist-ds-sw",a:"DranStrike 1-50 Low Rush",b:"SilverWolf 9-60 Hexa",aWins:7,bWins:5,aSpin:2,aBurst:0,aOver:12,aXtreme:5,bSpin:22,bBurst:0,bOver:4,bXtreme:0},
{id:"hist-ds-pw",a:"DranStrike 1-50 Low Rush",b:"PhoenixWing 9-60 Gear Flat",aWins:7,bWins:5,aSpin:12,aBurst:1,aOver:6,aXtreme:3,bSpin:10,bBurst:0,bOver:2,bXtreme:3}
];
...
async function seed(){
 ...
 if(!(await getAll("tests")).length) await put("tests",{id:"test-ds-bb",focusId:"build-ds",opponentId:"build-bb",target:12,status:"Active",priority:"High",purpose:"Validate DranStrike against balance/counter"});
}
...
function nav(){return `<nav>${[["home","⌂","Home"],["wizard","＋","New Test"],["battle","⚔","Battle"],["tests","☷","Tests"],["analytics","▥","Stats"],["collection","⚙","Collection"]].map(x=>`<button class="${state.page===x[0]?"active":""}" data-go="${x[0]}"><span>${x[1]}</span>${x[2]}</button>`).join("")}</nav>`}
function badge(text,level=""){return `<span class="badge ${level}">${text}</span>`}
function render(){
 const views={home:homeView,wizard:wizardView,battle:battleView,tests:testsView,analytics:analyticsView,collection:collectionView};
...
function collectionView(){
 ...
 <div class="card"><h2>Saved builds and history</h2>${state.builds.map(b=>`<div class="queue-item"><div class="queue-top"><div><b>${b.name}</b><br><span class="muted">${b.role||""}</span></div>${badge(b.status||"Candidate")}</div><p class="tiny">${b.blade} · ${b.ratchet} · ${b.bit}</p><div class="actions"><button class="btn secondary" data-edit-build="${b.id}">Edit</button><button class="btn danger" data-delete-build="${b.id}">Delete</button></div></div>`).join("")}</div>
...
async function startTest(id){const t=state.tests.find(x=>x.id===id),a=build(t.focusId),b=build(t.opponentId),err=pairIssue(a,b);if(err)return alert(err);state.battle={id:uid(),testId:id,a:a.name,b:b.name,aScore:0,bScore:0,rounds:[],startedAt:new Date().toISOString()};state.page="battle";render()}
...
async function saveMatch(){const m=state.battle;m.winner=m.aScore>m.bScore?m.a:m.b;m.date=new Date().toISOString();await put("matches",m);const t=state.tests.find(x=>x.id===m.testId);if(t&&completed(t)+1>=t.target){t.status="Complete";await put("tests",t)}state.battle=null;state.page="home";await refresh()}
